...
`slink` stores no state in local files; all configuration (including auth tokens) are instead read from environment variables. Using these variables, `slink` can be configured to make direct calls to XRPC services and it can also connect to remote services through a local proxy such as [IO](https://agent.io/posts/io) which handles routing and authentication.
